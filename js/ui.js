...
import { getSubtasks, getSubtaskStats } from './taskManager.js';
...
export function renderTaskList(tasks) {
  ...
  parentTasks.forEach(task => {
    ...
    if (subtasks.length > 0) {
      const subtaskContainer = createSubtaskContainer(task.id, subtasks);
      fragment.appendChild(subtaskContainer);
    }
  });
...
function createTaskElement(task) {
  ...
  const stats = !task.parentId ? getSubtaskStats(task.id) : null;
  ...
  li.innerHTML = `
    ...
    <div class="task-content">
      ...
      <div class="task-meta">
        ...
        ` : ''}
        ${stats && stats.total > 0 ? `
          <span class="subtask-progress" aria-label="Subtask progress: ${stats.completed} of ${stats.total}">
            <svg class="icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
    ...
    <div class="task-actions">
      ${!task.parentId ? `
        <button
          type="button"
          class="btn-icon"
          aria-label="Add subtask to: ${escapeHtml(task.title)}"
...
/**
 * Create a subtask element (simplified version of task element)
 * @param {Object} subtask - Subtask object
 * @returns {HTMLElement} Subtask element
 */
function createSubtaskElement(subtask) {
  const li = document.createElement('div');
  li.className = `subtask-item ${subtask.status === 'completed' ? 'completed' : ''}`;
  li.setAttribute('data-task-id', subtask.id);
  li.setAttribute('data-parent-id', subtask.parentId);
  ...
  li.innerHTML = `
    ...
    <div class="subtask-actions">
      <button
        ...
        aria-label="Edit subtask: ${escapeHtml(subtask.title)}"
        data-action="edit"
      >
      ...
      <button
        ...
        aria-label="Delete subtask: ${escapeHtml(subtask.title)}"
        data-action="delete"
      >
        <svg class="icon" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
          <polyline points="3 6 5 6 21 6"></polyline>
